...
class Parser:
    ...
    def parse_statement(self):
        ...
        if tok.value == "FUNCTIE":
            ...
            body = self.parse_block()
            self.expect("KEYWORD", "SFARSIT")
            return FunctionDefInstr(name, params, body)

        if tok.value == "RETURNEAZA":
            self.eat()
            return ReturnInstr(self.parse_expr())
        ...
        if tok.type == "IDENT":
            ...
            if self.cur().type == "OP" and self.cur().value == "[":
                ...
                self.expect("OP", "]")
                self.expect("OP", "<-")
                return ArrayAssignInstr(name, idx, self.parse_expr())
            self.expect("OP", "<-")
    ...
    def parse_factor(self):
        ...
        raise Exception(f"Factor invalid: {tok}")
